...
// ----------------------
// Rule-based gesture detection (replaces missing trained model)
// Triggers when both hands are close together (jutsu seal position)
// ----------------------
let gestureModel = true; // always "loaded" so the trigger check runs

function handsClose(right, left) {
  // Check if wrists/palms are within ~25% of frame width of each other
  const dx = right[0].x - left[0].x;
  const dy = right[0].y - left[0].y;
  return Math.sqrt(dx * dx + dy * dy) < 0.25;
}

function predictGesture(right, left) {
  if (!right || !left) return false;

  const close = handsClose(right, left);
  const prob = close ? 1.0 : 0.0;
